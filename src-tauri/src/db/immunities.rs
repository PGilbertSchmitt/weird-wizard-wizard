...
use crate::{WWResult, import::NameToId};
...
impl Immunity {
    pub async fn insert_all(tx: &mut SqliteConnection, immunities: &HashSet<String>) -> WWResult<NameToId> {
        let mut name_to_id = NameToId::new();
        // Linear execution is probably fine for now
        for immunity in immunities {
            let label = immunity.clone();
            let record = sqlx::query!(
                "INSERT INTO immunities (name) VALUES (?)",
                immunity,
            )
                .execute(&mut *tx)
                .await?;
