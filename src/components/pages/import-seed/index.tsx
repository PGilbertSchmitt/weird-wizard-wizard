                    ...
                    if (prev.status === ImportStatuses.IDLE) {
                        const unlistenPromise = listen(
                            ...
                            (event: Event<IpcResult<ImportEvent>>) => {
                                try {
                                    const payload = unwrapIpcResult(
                                        event.payload,
                                    );
                                    switch (payload.type) {
                                        case 'Ready': {
                                            console.log(
                                                `${payload.data.ancestries} ancestries`,
                                            );
                        ...
                        unlistenPromise.then(() => {
                            console.log(
                                `Invoking "init_seed" signal with ${data}`,
                            );
                            invoke('init_seed', { filepath: data }).catch(
                                (err) => {
                                    console.error(err);
                                    dispatch(cancelAction());
                                },
        ...
        <>
            ...
            <div className={cn('flex flex-col items-center max-w-100')}>
                <Paragraph>
                    ...
                    -specific information that is needed to create a character.
                    This includes things like ancestries, paths, traditions, and
                    spells. This tool does not ship with this data because it's
                    not my data to distribute (it belongs to Robert J. Schwalb
                    and{' '}
                ...
                <Paragraph>
                    If you have zip file of the seed data, you can drop it off
                    here. If you don't have one yet, you can create one if you
                    have the{' '}
