import { LanguageRecord } from '../import-data/natural-attribute-import';
...
export const createImmunity = async (name: string) => {
  return id(
    Tables.IMMUNITIES,
    await dbExecute(`INSERT INTO ${Tables.IMMUNITIES} (name) VALUES ($1)`, [
      name,
    ]),
  );
};

const createNameDescUnit =
  (table: TableName) =>
  async (record: { name: string; description: string; unit: string | null }) =>
    id(
      table,
      await dbExecute(
        `
